...
// 引入电影模型
Category = require('../../models/movie/movie_category'),
// 引入电影分类模型
City = require('../../models/movie/movie_city'),
// 引入电影院模型
...
exports.index = function(req, res) {
        var _galleryName = req.query.galleryName,
        // 获取正在上映和即将上映播放标题名
        _fliterName = req.query.fliterName,
        // 选电影/选电视剧区电影分类标题名称
        ...
        if (_cityName) {
                ...
                Category.find({}).populate({
                        ...
                        City.find({}).exec(function(err, cinemas) {
                                ...
                                CityProgramme.find({}).populate('cityCategories', 'name').exec(function(err, cityProgrammeList) {
                                        if (err) {
                                                console.log(err);
                                        }
                                        CityCategory.find({}).populate('cities', 'name').populate('cityProgramme', 'name').exec(function(err, cityCategoryList) {
                                                if (err) {
                                                        console.log(err);
                                                }
