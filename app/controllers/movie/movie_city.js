...
var City = require('../../models/movie/movie_city'),
// 电影影院数据模型
CityCategory = require('../../models/movie/movie_cityCategory'),
// 电影院分类模型
CityProgramme = require('../../models/movie/movie_cityProgramme'); // 电影院分类股归类
// 电影院录入页面渲染控制器
exports.new = function(req, res) {
        CityCategory.find({}).exec(function(err, cityCategories) {
                if (err) {
                        console.log(err);
                } else {
...
exports.save = function(req, res) {
        var cityObj = req.body.city,
        // 获取城市录入页发送的数据
        cityName = cityObj.name,
        // 新建城市名称
        cinemas = cityObj.cinemas,
        // 新建电影院名称
        cityCategoryName = cityObj.cityCategoryName,
        // 新建城市分类名称
        cityCategoryId = cityObj.cityCategoryId,
        // 所选城市ID
        cityProgrammeName = cityObj.cityProgrammeName,
        // 新建城市分类归类名称
        cityProgrammeId = cityObj.cityProgrammeId; // 所选分类归类的ID
        // 建立城市分类和归类之间的联系  比如A <-> A-G
        if (!cityName && cityProgrammeId && cityCategoryId) {
                CityProgramme.findById(cityProgrammeId,
                function(err, _oldCityProgramme) {
                        CityCategory.findById(cityCategoryId,
                        function(err, _oldCityCategory) {
        ...
        if (cityProgrammeName) {
                CityProgramme.findOne({
                        name: cityProgrammeName
                },
                function(err, _cityProgramme) {
                        if (err) {
                                console.log(err);
                        }
                        // 如果归类已存在
        ...
        // 如果输入了城市分类名称
        if (cityCategoryName) {
                CityCategory.findOne({
                        name: cityCategoryName
                },
                function(err, _cityCategory) {
                        if (err) {
                                console.log(err);
                        }
                        // 城市分类已存在
                        ...
                        if (_city && _city.cinemas) {
                                ...
                                newCity.save(function(err, _newCity) {
                                        ...
                                        for (var i = 0; i < cityCategoryArray.length; i++) {
                                                CityCategory.findById(cityCategoryArray[i],
                                                function(err, _oldCityCategory) {
                                                        if (_oldCityCategory) {
...
exports.list = function(req, res) {
        CityCategory.find({}).populate('cities', 'name cinemas').populate('cityProgramme', 'name').exec(function(err, cityCategoryList) {
                if (err) {
                        console.log(err);
                }
