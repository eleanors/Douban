...
// 引入音乐分类模型
Programme = require('../../models/music/music_programme'),
// 引入近期热门歌单区域模型
fs = require('fs'),
// 读写文件模块
...
exports.index = function(req, res) {
        ...
        // 获取新碟榜区分类请求名称
        hotProName = req.query.hotProName,
        // 获取近期热门歌单分类请求名称
        ...
        if (albumName) {
                ...
                // 音乐分类查找
                MusicCategory.find({}).populate('musics').exec(function(err, musicCategories) {
                        if (err) {
                                console.log(err);
                        }
                        //近期热门歌单区域歌曲分类查找
                        Programme.find({}).populate({
                                path: 'musicCategories',
                                select: 'name musics'
...
exports.search = function(req, res) {
        ...
        if (catId) {
                ...
                MusicCategory.find({
                        ...
                        if (musicCategories) {
                                var musicCategory = musicCategories[0] || {},
                                // 查询到的音乐分类
                                musics = musicCategory.musics || [],
                                // 分类中包含的音乐
                                results = musics.slice(index, index + count); // 分类页面每页显示的音乐数量
                                res.render('music/music_results', {
                                        title: '豆瓣音乐分类列表页面',
                                        keyword: musicCategory.name,
                                        // 分类名称
                                        currentPage: (page + 1),
                                        // 当前页
                                        query: 'cat=' + catId,
                                ...
                                for (var i = 0; i < len; i++) {
                                        MusicCategory.findOne({
                                                ...
                                                if (count === len) {
                                                        ...
                                                        res.render('music/music_results', {
                                                                title: '近期热门歌单分类列表页面',
                                                                keyword: musicCategories[0].name,
                                                                // 分类名称
                                                                currentPage: (page + 1),
                                                                // 当前页
                                                                query: 'pro=' + proId,
                                                                // 切换到另一页
                                                                totalPage: Math.ceil(dataMusics.length / count),
                                                                // 总页数，需向上取整
