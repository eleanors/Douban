...
//音乐数据模型
MusicComment = require('../../models/music/music_comment'),
//音乐评论模型
MusicCategory = require('../../models/music/music_category'),
//音乐分类模型
...
exports.detail = function(req, res) {
        ...
        function(err, music) {
                ...
                MusicComment.find({
                        music: _id
                }).populate('from', 'name').populate('reply.from reply.to', 'name') // 查找评论人和回复人的名字
...
exports.savePoster = function(req, res, next) {
        var imageData = req.files.uploadMusicImage,
        //上传文件
        ...
        if (originalFilename) {
                ...
                function(err, data) {
                        ...
                        //获取时间
                        type = imageData.type.split('/')[1],
                        //获取图片类型 如jpg png
                        image = timestamp + '.' + type,
                        //上传海报新名字
...
exports.save = function(req, res) {
        ...
        if (id) {
                ...
                _music.save(function(err, music) {
                        ...
                        // 选择了音乐所属的音乐分类
                        if (musicCategoryId) {
                                MusicCategory.findById(musicCategoryId,
                                function(err, musicCategory) {
                                        if (err) {
                                                console.log(err);
                                        }
                                ...
                                MusicCategory.findOne({
                                        name: musicCategoryName
                                },
                                function(err, _musicCategoryName) {
                                        if (err) {
                                                console.log(err);
                                        }
...
exports.list = function(req, res) {
        Music.find({}).populate('musicCategory', 'name').exec(function(err, musics) {
                if (err) {
                        console.log(err);
                }
...
exports.del = function(req, res) {
        ...
        if (id) {
                Music.findById(id,
                function(err, music) { // 查找该条音乐信息
                        if (err) {
                                console.log(err);
                        }
                        // 查找包含这条音乐的音乐分类
                        MusicCategory.findById(music.musicCategory,
                        function(err, musicCategory) {
                                if (err) {
                                        console.log(err);
                                }
